...
| Security review | Partial | `unsafe_code = "forbid"` is a workspace-wide lint (`Cargo.toml`), and `cargo clippy --workspace --all-targets -- -D warnings` is clean. `cargo-audit` (v0.22.2) was installed and run against the full 221-crate dependency graph on 2026-07-31: `cargo audit` and `cargo audit --deny warnings` both exit 0 against the RustSec advisory database (1177 advisories loaded from `https://github.com/RustSec/advisory-db.git`) with no known vulnerability, unmaintained, unsound, or yanked-crate finding. Fuzzing was still not attempted; recommended before a public release rather than completed here. |
...
rather than hidden: partial security/performance review (`cargo audit`
now run clean against all 221 dependencies, but no fuzzing and no
benchmark suite), unwritten architecture documentation
